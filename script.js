...
const weeklyWorkoutForm = document.getElementById('weeklyWorkoutForm');
const weeklyResultsSection = document.getElementById('weeklyResults');
const weeklyPlanSummary = document.getElementById('weeklyPlanSummary');
const weeklyWorkoutPlan = document.getElementById('weeklyWorkoutPlan');
...
if (weeklyWorkoutForm) {
  weeklyWorkoutForm.addEventListener('submit', handleWeeklyFormSubmit);
}
if (downloadWeeklyPlanBtn) {
  downloadWeeklyPlanBtn.addEventListener('click', downloadWeeklyPlan);
}
if (emailWeeklyPlanBtn) {
  emailWeeklyPlanBtn.addEventListener('click', emailWeeklyPlan);
}

function getSelectedWeeklyEquipment() {
  const checkboxes = document.querySelectorAll('.weekly-equipment');
  const selected = [];
  checkboxes.forEach(checkbox => {
...
function createWeeklyOpenAIPrompt(formData) {
  const goalLabels = {
  ...
  return `Create a detailed, personalized weekly workout plan in HTML format for a user with the following profile:\n\n- Primary Goal: ${goalLabels[formData.goal] || formData.goal}\n- Days per Week: ${formData.days}\n- Available Equipment: ${formData.equipment.map(eq => equipmentLabels[eq] || eq).join(', ') || 'None'}\n- Preferences or Limitations: ${formData.preferences || 'None'}\n\nRequirements:\n1. Provide a ${formData.days}-day plan, with each day including:\n   - Warm-up routine (5-10 min)\n   - Main workout (exercises, sets, reps, rest)\n   - Cooldown/stretching (5-10 min)\n2. Use only the available equipment.\n3. Vary the workouts through the week.\n4. Add form tips and safety notes where relevant.\n5. Output in clean, semantic HTML with headings for each day, exercise lists, and highlight tips/notes.\n6. Make the plan realistic and actionable.\n`;
}
...
async function handleWeeklyFormSubmit(e) {
  ...
  try {
    ...
    setWeeklyLoadingState(false);
  }
}

function displayWeeklyWorkoutPlan(formData, aiResponse) {
  // Summary
  const goalLabels = {
    'weight-loss': 'Weight Loss',
    'muscle-building': 'Muscle Gain',
  ...
  weeklyPlanSummary.innerHTML = `
    <div class="summary-card"><strong>Goal:</strong> ${goalLabels[formData.goal]}</div>
    <div class="summary-card"><strong>Days/Week:</strong> ${formData.days}</div>
    <div class="summary-card"><strong>Equipment:</strong> ${formData.equipment.map(eq => equipmentLabels[eq] || eq).join(', ') || 'None'}</div>
    <div class="summary-card"><strong>Preferences:</strong> ${formData.preferences || 'None'}</div>
  `;
  weeklyWorkoutPlan.innerHTML = aiResponse;
  weeklyResultsSection.classList.remove('hidden');
  weeklyResultsSection.scrollIntoView({ behavior: 'smooth' });
}

function setWeeklyLoadingState(loading) {
  const btn = document.querySelector('.weekly-generate-btn');
...
function downloadWeeklyPlan() {
  // Export the plan as PDF (using jsPDF if available, else fallback to text)
  if (window.jspdf || window.jsPDF) {
    const doc = new (window.jspdf || window.jsPDF)();
    doc.html(weeklyWorkoutPlan, {
    ...
    const text = weeklyWorkoutPlan.innerText || weeklyWorkoutPlan.textContent;
    const blob = new Blob([text], { type: 'text/plain' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'weekly-workout-plan.txt';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
...
function emailWeeklyPlan() {
  // Use mailto: with the plan as body (plain text)
  const subject = encodeURIComponent('My Weekly Workout Plan');
  const body = encodeURIComponent((weeklyWorkoutPlan.innerText || weeklyWorkoutPlan.textContent).slice(0, 2000));
